      ...
      >
        ...
        >
          {cards.map((imgUrl, i) => (
            <motion.div
              key={`key-${imgUrl}-${i}`}
              className="absolute flex h-full origin-center items-center justify-center rounded-xl bg-black p-2"
              style={{
                width: `${faceWidth}px`,
                transform: `rotateY(${
                  i * (360 / faceCount)
...
export function ThreeDPhotoCarousel() {
  ...
  const cards = useMemo(
    () => keywords.map((keyword) => `https://picsum.photos/500/500?${keyword}`),
    []
  )
